...
from jgmp.cardinality_configs import watcher_config
from jgmp.cardinality_estimator.learned_cardinality_estimator import LearnedCardinalityEstimator
from jgmp.cardinality_estimator.postgres_cardinality_estimator import PostgresCardinalityEstimator
from jgmp.cardinality_estimator.prophetic_alias_cardinality_estimator import PropheticAliasCardinalityEstimator
from jgmp.cardinality_injection import parse_get_runtime_sql, extract_parse_subplans, get_data_directory, create_subplan_order_files, save_estimations, execute_query
from jgmp.cardinality_test import train_test, baseline_runtimes, SubqueryMode
from jgmp.plan_execution.pg_explain import PgExplain
from jgmp.plan_execution.stubborn_plan_engine import StubbornPlanEngine
from jgmp.query.query_generator_choice import QueryGeneratorChoice
from jgmp.query.sql.sql_query import SQLQuery
from jgmp.query.symmetry.eliminate_cross_product import detect_cross_product
from jgmp.schema.schemas import dsb_schema
from jgmp.schema.sql.sql_schema import SQLSchema
from jgmp.training_test_split import multi_split
...
def dsb_test(query_directory: str,
    ...
    # queries = get_queries(query_directory, queries_per_template=queries_per_template)
    # note: queries = [(query_name, query_in_str)]
    queries = [
        ("query", "SELECT * FROM store_sales s1 WHERE s1.ss_sales_price > 10 AND s1.ss_sales_price < 20"),
        ("query1", "SELECT * FROM store_sales s1 WHERE s1.ss_sales_price > 15 AND s1.ss_sales_price < 25")
    ]
    ...
    if subquery_mode == SubqueryMode.BEST:
        ...
        best_subplans_dict = {}
        for query_name, query_in_str in queries:
            sql_query = SQLQuery.parse_sql(schema, query_in_str)
            sql_query.cardinality_estimates()["true"] = 99
            best_subplans_dict[query_name] = [sql_query]
    print(best_subplans_dict)
    ...
    for model_no, (training_queries, test_queries) in enumerate(splits):
        if subquery_mode == SubqueryMode.BEST:
            ...
            # query_generator = QueryGeneratorChoice(sum([all_subplans_dict[query_name] for query_name in training_queries], [])) # a list of all the queries
            query_generator = QueryGeneratorChoice(sum(training_set_pure, []))
        ...
        for config in configs:
            ...
            for query_name, query in test_queries:
                ...
                print("!!!", estimator.estimate(SQLQuery.parse_sql(schema, query)))
...
def watcher_dsb_train(
    list_of_name_and_sql_and_truecard: List[Tuple[str, str, int]],
    port: int = 5555
):
    config = watcher_config()
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    schema = SQLSchema.sql_schema_from_connection("watcherdb", fk_name="jgmp-dsb-schema", port=port)

    # note: queries = [(query_name, query_in_str)]
    queries = [x[:2] for x in list_of_name_and_sql_and_truecard]

    start_ts = time.time()
    best_subplans_dict = {}
    for i, (query_name, query_in_str) in enumerate(queries):
        sql_query = SQLQuery.parse_sql(schema, query_in_str)
        sql_query.cardinality_estimates()["true"] = list_of_name_and_sql_and_truecard[i][-1]
        best_subplans_dict[query_name] = [sql_query]
    print("JGMP parsing time:", time.time() - start_ts)

    training_set_pure = [best_subplans_dict[query_name] for query_name in best_subplans_dict]
    query_generator = QueryGeneratorChoice(sum(training_set_pure, []))

    start_ts = time.time()
    _, _, model = train_test(schema, training_set_pure, [], device, config, validate_ratio=0, query_generator=query_generator)
    estimator = LearnedCardinalityEstimator(model, device)
    print("JGMP training time:", time.time() - start_ts)
    return schema, estimator

def watcher_dsb_test(
    schema: SQLSchema,
    estimator: LearnedCardinalityEstimator,
    sql_in_str: str
):
    parsed_sql = SQLQuery.parse_sql(schema, sql_in_str)
    return int(round(estimator.estimate(parsed_sql)))
