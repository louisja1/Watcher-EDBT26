...
class SQLSchema(GraphlikeSchema):
    ...
    def connection(self, timeout: Optional[int] = None):
        if timeout is None:
            ...
            return psycopg2.connect(host="localhost", database=self._name, user="yl762", options='-c statement_timeout=%d' % timeout_ms)
    ...
    def sql_schema_from_connection(name: str, schema: str = "public", leafs: List[str] = [], mask: Optional[Set[str]] = None, fk_name: Optional[str] = None, port: Optional[int] = None) -> SQLSchema:
        if port is None:
            connection = psycopg2.connect(host="localhost", database=name, user="yl762")
        else:
            connection = psycopg2.connect(host="localhost", database=name, user="yl762", port=port)
        if fk_name is None:
            fk_connection = connection
        elif port is None:
            fk_connection = psycopg2.connect(host="localhost", database=fk_name, user="yl762")
        else:
            fk_connection = psycopg2.connect(host="localhost", database=fk_name, user="yl762", port=port)
        connection.set_session(readonly=True, autocommit=True)
        fk_connection.set_session(readonly=True, autocommit=True)
