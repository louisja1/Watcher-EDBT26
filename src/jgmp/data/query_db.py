...
class QueryDB:
    def __init__(self, name: str, port: Optional[int] = None) -> None:
        if port is None:
            self._connection = psycopg2.connect(host="localhost", database=name, user="yl762")
        else:
            self._connection = psycopg2.connect(host="localhost", database=name, user="yl762", port=port)

        self._query_languages = self._get_name_dict("query_languages")
        print(self._query_languages)
