    ...
    def parse_sql(schema: SQLSchema, sql: str) -> SQLQuery:
        ...
        for token in parsed.tokens:
            if token.is_keyword and token.normalized == "FROM":
                next_from = True
            elif (isinstance(token, IdentifierList) or isinstance(token, Identifier)) and next_from:
                next_from = False
                if isinstance(token, IdentifierList):
                    ...
                    if isinstance(token, Identifier):
                        table = next(it for it in token.tokens if it.ttype == Name).normalized.lower()
                        short = token.tokens[-1].normalized.lower()
                        tables[short] = schema.node(table)
                        predicates[short] = []
