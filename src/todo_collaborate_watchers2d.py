    ...
    with open(trainingset_filename, "r") as fin:
        ...
        if two_way_model_name is not None:
            ...
            black_box.retrain(
                ...
                two_way_model_joins,
                two_way_training_set,
                f"select * from " + ", ".join(two_way_model_tables) + " where " + " and ".join(two_way_model_joins) + f" and {list(sel_cols[two_way_model_tables[0]].keys())[0]} between {{lb1}} and {{ub1}} and {list(sel_cols[two_way_model_tables[1]].keys())[0]} between {{lb2}} and {{ub2}};",
            )
