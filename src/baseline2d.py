...
def load_trainingset(
    ...
    with open(trainingset_filename, "r") as fin:
        ...
        black_box.retrain(
            ...
            tab_to_training_set,
            f"select * from " + ", ".join(baseline2d.model_tables) + " where " + " and ".join(baseline2d.model_joins) + f" and {list(sel_cols[baseline2d.model_tables[0]].keys())[0]} between {{lb1}} and {{ub1}} and {list(sel_cols[baseline2d.model_tables[1]].keys())[0]} between {{lb2}} and {{ub2}};",
        )
